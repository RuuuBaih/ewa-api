...
module JustRuIt
  ...
  class GmapPlaceDetailsApi
    ...
    class Yamlfile
      def initialize(place_details_file)
        @place_file = place_details_file
      end

      def save_as_yaml_file
        File.open('../../../spec/fixtures/gmap_place_data/place_details_results.yml', 'w') do |file|
          file.write(@place_file.to_yaml)
        end
      end
    end
